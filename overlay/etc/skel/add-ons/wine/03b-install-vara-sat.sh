...
# Purpose : Install VARA SAT
source ./common-checks.sh

VARA_HOME="$HOME/.wine32/drive_c/VARA SAT"
VARA_PATTERN="VARA%20SAT"

./vara-downloader.sh "${VARA_PATTERN}"
[[ $? -ne 0 ]] && et-log "Error downloading VARA SAT" && exit 1
...
wine 'VARA SAT setup (Run as Administrator).exe'
...
echo -e "${YELLOW}Run ${WHITE}./03c-install-vara-terminal.sh${YELLOW} to install VARA Terminal.${NC}"
